---
title: International Fiance 
tags: 国际金融，汇率汇率
grammar_cjkRuby: true
---
# 第1章NI & BoP
- 资产：财富的一种形式，一种把购买力从现在转向将来的手段。
# 第2章 汇率和外汇市场
- 汇率exchange rate：一国货币用另一国货币表示的价格，USD/EUR=0.81。也是一种资产价格。
## 学习目标
1. 关键术语：升值，贬值，套利，汇率，外汇市场，远期汇率，利率，流动性，升值率，贬值率，收益率，实际收益率，风险，即期利率，载体货币，银行间交易，汇率平价条件
2. 将ER变动与一国出口产品的相对价格变动联系在一起。
2. 描述外汇市场的结构和功能。
3. 利用汇率计算并比较以不同货币计价的资产回报。
4. 利用利率平价找到均衡汇率。
5. 找到利率与其期望变动对汇率的影响。

## 汇率和国际交易
1. 直接报价（美式标价）美元/欧元，每欧元的美元数额与间接标价（欧式）
2. 贬值升值

## 外汇市场
1. 汇率是由买卖外汇进行国际支付的家庭、企业和金融机构的相互作用决定的。
2. 外汇市场：国际货币的交易场所。
3. 参与者：商业银行，公司，非银行金融机构（养老基金、保险公司），中央银行（作用大）
4. 套利arbitrage：对货币的贱买贵卖。为此，金融中心一体化。
5. 美元：载体货币vehicle currency
6. 即期汇率spot exchange rate：立即交割transaction；远期汇率forward：指定在未来一个时间上交割，交割日，保值。
7. 远期汇率是如何决定的，远期汇率和即期汇率变动之间关系密切的原因？
8. 外汇掉期交易：即期卖出一种货币同时远期再买回这种货币。比两次外汇交易少花一些经纪人佣金。
9. 期货与期权

## 外汇资产需求
1. 收益率rate of return；预期收益率；预期实际收益率：减去机会成本的收益率
2. 无本金交割的亚洲远期外汇交易：许多发展中国家的货币无法自由兑换，人民币只能在中国境内由居民进行交易，而不能在境外交易，所以无法进行传统的远期外汇交易。
3. 风险risk和流动性liquidity
4. 利率interest rate：借出1单位的某种货币一年可以获得的该种货币数量，计算某种货币存款的收益率。外汇市场的参与者需要知道：存款的货币价值将如何改变；汇率如何变化以便将不同货币计量的收益率进行转换和比较。
5. 美元相对与欧元的贬值率depreciation：美元/欧元汇率的年增长率。法则：欧元存款的美元收益率=欧元的利率+美元相对欧元的贬值率。换言之，把欧元存款的欧元收益转化成美元收益，需要在欧元利率的基础上+欧元的美元价格的年上涨率
6. **求预期收益率（公式角度）：两国利率，对未来汇率的预期，当前的汇率**
7. 假设外汇市场参与者对外汇资产的需求完全取决于：对不同资产预期收益率的比较

## 外汇市场均衡
1. 如果市场在某个汇率水平上出清，那么在该汇率水平上，市场参与者将愿意持有现有的所有各种货币存款。
2. 外汇市场参与者如何形成对未来汇率的预期？（3、4章）
3. 当**所有**的货币存款都提供相同的预期收益率时，外汇市场处于均衡状态。
3. **利率平价**interest parity condition：用相同货币衡量的任意两种货币存款的预期收益率相等的条件
4. 当前汇率变动如何影响预期收益（先影响外汇存款的预期本币收益率）：从公式角度/直观角度
5. 均衡理论的决定：汇率总是在不断调整以维持利率平价。收益率低——贬值

## 利率、预期与均衡
1. 之前看做不变的利率和预期汇率，现在变动，如何影响当前汇率？*跟之前反过来了*仍然直接公式法，其他变量不变。直观感受：美元利率上升，美元升值；预期汇率上升，欧元升值。
2. 套利交易：国际投资者借入低利率货币（融资货币），买入高利率货币（投资货币），在长期内可以盈利。那么利率平价是错的吗？因为风险非常高，可能澳元突然崩溃。

## 第2章附录：远期汇率与抵补的利率平价
strong text
# 第3章 货币、利率与汇率
- 建立经济模型研究货币的汇率取决因素。
- 第一步：弄清一国的货币S和P对利率和汇率的影响。
- 第二步：货币供求平衡是如何决定均衡利率的。
## 学习目标
1. 关键术语：货币总需求，通货紧缩，汇率超调，通货膨胀，长期，长期均衡，货币供给，价格水平，短期
## 货币定义
## 个人货币需求
## 货币总需求
## 均衡利率
## 短期货币供给和汇率
## 长期货币、价格水平和汇率
## 通货膨胀和汇率的动态变化

# 第4章 长期价格水平和汇率
- 购买力平价理论PPP：两个国家的价格水平变化是如何引发两种货币之间的汇率波动的
- PPP可能无法作出长期预测的原因，有时候要参考产品市场
- 拓展后的购买力平价理论

## 学习目标
1. 关键术语：绝对购买力平价，费雪效应，一价定律，汇率的货币分析法，名义汇率，名义利率，对市场定价，购买力平价，实际升值，实际贬值，实际汇率，实际利率，相对购买力平价
2. 解释汇率的PPP理论及PPP与国际商品市场一体化之间的关系
3. 长期内，货币因素如何影响汇率
4. 实际汇率的概念
5. 长期内影响实际汇率与相对货币价值的因素
6. 国际实际利率差异与实际汇率的预期变动

## PPP
1. 一价定律law of one price：在没有运输费用和官方贸易壁垒（关税）的自由竞争市场上，同样的货物在不同国家出售，按同一货币计量的价格应该是一样的。揭示了国内商品价格和汇率之间的基本联系。
2. 购买力平价理论PPP：两国货币的汇率等于两国价格水平之比。19世纪
3. 一国的价格水平，即一个基准商品和服务篮子的价格，反映了该国货币的**国内购买力**。某种货币**国内购买力**的下降，会表现为国内价格水平的上升，会引起该货币在外汇市场上的等比例贬值。
4. PPP与LOP：两式类似。差别：LOP适用于单个商品i的情况；PPP适用于普遍的价格水平。PPP不一定要求LOP一定成立，因为即使单个商品不成立，P和ER也不会偏离太远。
5. 绝对购买力平价absolute PPP：两国货币的汇率等于两国价格水平之比，反映价格水平和利率水平的关系；相对购买力平价relative PPP：两国汇率变化的百分比=同一时期两国国内价格水平变化的百分比之差（通货膨胀率之差），反映价格水平变动与利率水平变动，**要指明价格水平与汇率发生变化的那一段时间**。
6. 为什么要用rPPP？因为一国政府通常不会花费精力去采用国际标准的商品篮子来计算本国的价格指数，这使得aPPP没有意义。我们用政府公布的物价统计资料评估PPP时，rPPP就很有用。

## 建立在PPP之上的长期汇率模型
## PPP与LOP的经验证明
## 如何解释PPP出现的问题
1. LOP存在的问题：运输费用和贸易管制是存在的；商品市场的垄断或寡头行为可能会与运输费用和贸易壁垒相互作用，进一步削弱不同国家商品价格之间的联系；不同国家公布的通货膨胀数据依据的商品篮子不同，因此汇率变动不可能抵消官方所衡量的通货膨胀差异。
2. 运输成本和贸易管制：切断了LOP下汇率和商品价格之间的密切联系。
3. 非贸易品：相对于一些商品和服务的生产成本而言，运输成本有时可以高到使得这些商品的国际流通无利可图。理发。非贸易品的价格完全是由国内的供求关系决定的，衡量各国价格水平的基准商品篮子通常包含多种非贸易品，所以它的价格上涨会提高本国价格水平。可以解释实际情形与PPP地大幅度偏离。
4. 贸易品：制成品、原材料、农产品；非贸易品：服务和建筑行业产品（金融服务，互联网除外）。
5. 非自由竞争：不完全竞争市场结构中，极端情况下，一家公司能够在不同的市场上对市场定价pricing to market
6. **一种货币被高估了**：美式汇率，同种商品，以美元表示的购买力平价是真实值，用标定的汇率比较真实值。
## 超越PPP：长期汇率的一般模型
## 国际利差与实际利率
## 实际利率平价
## 第4章附录：利用弹性价格货币分析法研究费雪效应、利率和汇率